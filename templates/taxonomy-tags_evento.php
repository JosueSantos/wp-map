...
$comunidade_ids = $term instanceof WP_Term ? cc_get_comunidade_ids_by_event_taxonomy('tags_evento', $term->term_id) : [];
...
<main class="bg-slate-50 min-h-screen py-10">
    <section class="max-w-6xl mx-auto px-4 space-y-8">
        <header class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 lg:p-8 space-y-3">
            <p class="text-sm text-slate-500">Característica de evento</p>
            <h1 class="text-3xl font-bold text-slate-900">Locais com <?php echo esc_html($term->name ?? 'esta característica'); ?></h1>
        ...
        <?php if (empty($comunidade_ids)): ?>
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                <p class="text-slate-600">Nenhum local possui eventos com esta característica.</p>
            </div>
        <?php else: ?>
